...
# video_writer = cv.VideoWriter('test_jake.mp4', cv.VideoWriter_fourcc(*'avc1'), fps, (width, height))
images = {}
...
while not shutdown:
    ...
    try:
        while not image_messages.empty():
            ...
            data_time = img_data[0]
            key = img_data[1]
            picture = img_data[2]
            if key not in images.keys():
                images[key] = [picture]
            else:
                images[key].append(picture)
    ...
    if min_frames > 0:
        video_writer = cv.VideoWriter('test.mp4', cv.VideoWriter_fourcc(*'avc1'), fps, (width, height))
        try:
            for i in range(min_frames):
                frames = [value.pop(0) for key, value in images.items()]

                # Ensure all frames have the same dimensions
                resized_frames = [cv.resize(frame, (width, height)) for frame in frames]

                # Concatenate frames horizontally
                image = np.concatenate(resized_frames, axis=1)

                # Display the concatenated image
                cv.imshow('Image', image)
                cv.waitKey(1)  # Wait for 100 milliseconds
